...
class DecisionTreeCART:
    ...
    def _calculate_gini(self, data):
        ...
        gini_impurity = 1 - sum((count / num_samples) ** 2 for count in class_counts)
    ...
    def _traverse_tree(self, sample, node):
        ...
        for key, subtree in node[feature].items():
            if "<=" in str(key): 
                ...
                branch = "<= " + str(limit) if value <= limit else "> " + str(limit)
                if branch == key:
                    return self._traverse_tree(sample, subtree)
            elif "==" in str(key): 
                cat_value = str(key).split(" == ")[1]
                branch = "== " + cat_value if value == cat_value else "!= " + cat_value
                if branch == key:
                    return self._traverse_tree(sample, subtree)
